...
@REM 安定性のためCPU版PyTorchを使用（必要に応じてCUDA版に変更可能）
echo pip install -q torch^<2.4 torchvision^<0.19 torchaudio^<2.4 --index-url https://download.pytorch.org/whl/cpu
pip install -q torch^<2.4 torchvision^<0.19 torchaudio^<2.4 --index-url https://download.pytorch.org/whl/cpu
if %errorlevel% neq 0 ( pause & popd & exit /b 1 )
...
@REM PyAV Cythonエラー回避のため軽量版requirements使用
echo pip install -q -r requirements_no_pyav.txt
pip install -q -r requirements_no_pyav.txt
if %errorlevel% neq 0 ( pause & popd & exit /b 1 )

@REM NumPy互換性修正
echo pip install -q "numpy<2.0" "cython<3.0" --force-reinstall
pip install -q "numpy<2.0" "cython<3.0" --force-reinstall
if %errorlevel% neq 0 ( pause & popd & exit /b 1 )
