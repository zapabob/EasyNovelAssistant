...
from app.dialogs import ModelAddDialog
...
class ModelMenu:
    ...
    def on_menu_open(self):
        ...
        self.menu.add_command(label="ローカルLLMを追加", command=self.add_local_model)
    ...
    def add_local_model(self):
        ...
        try:
            with open(Path.llm, "r", encoding="utf-8-sig") as f:
                self.ctx.llm = json.load(f)
        except Exception as e:
            print(f"モデル設定の再読み込みに失敗しました: {e}")
