...
class InputArea:
    ...
    def get_file_path(self):
        return self.tab.file_path if self.tab else None

    def get_file_text(self):
        return self.tab.file_text if self.tab else None
    ...
    def get_text(self):
        return self.tab._get_text() if self.tab else ""

    def get_comment_removed_text(self):
        return self.tab._get_comment_removed_text() if self.tab else ""
    ...
    def set_chat_style(self):
        """チャットモード用のスタイル設定"""
        if self.tab and self.tab.text_area:
            self.tab.text_area.configure(height=3)
            # Enterキーでの送信を有効化
            self.tab.text_area.bind("<Return>", self._on_chat_enter)
            self.tab.text_area.bind("<Shift-Return>", self._on_chat_shift_enter)
        
    def set_novel_style(self):
        """ノベルモード用のスタイル設定"""
        if self.tab and self.tab.text_area:
            self.tab.text_area.configure(height=3)
        if self.tab and self.tab.text_area:
            self.tab.text_area.configure(height=20)
            # Enterキーの通常動作に戻す
            self.tab.text_area.unbind("<Return>")
            self.tab.text_area.unbind("<Shift-Return>")
    
    def _on_chat_enter(self, event):
        """チャットモードでのEnterキー処理"""
        if not event.state & 0x1:  # Shiftキーが押されていない
            if self.tab and self.tab.text_area:
                text = self.tab.text_area.get("1.0", "end-1c")
                self.ctx.form.process_chat_input(text)
            return "break"  # イベントの伝播を停止
            
    def _on_chat_shift_enter(self, event):
        """チャットモードでのShift+Enter処理"""
        # 改行を許可
        return None
    ...
    def clear_current(self):
        """現在のタブの入力内容をクリア"""
        if self.tab and self.tab.text_area:
            self.tab.text_area.delete("1.0", tk.END)
